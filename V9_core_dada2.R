...
err_V9_r1 <- learnErrors(filts_V9_r1, randomize = TRUE)
...
## Train parametric error model  
err_V9_r2 <- learnErrors(filts_V9_r2, randomize = TRUE)
...
taxa_V9_all <- assignTaxonomy(seqtab.nochim_V9_all, "Input/Seq/V9/silva_128.18s.99_rep_set.dada2.fa.gz",
                              tryRC = TRUE, minBoot = 80, multithread = TRUE)
...
write.csv(data.table(taxa.abundance.silva_V9), "Output/dada2/taxa.silva_V9_all_boot80.csv", row.names = FALSE)
...
write.csv(track_V9_all, file = "Output/dada2/track_reads_V9_80.csv", row.names = FALSE)
